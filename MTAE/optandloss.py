...
def optimize_thresholds_bayes(y_true, y_pred, mask=None, epsilon=0.2):
    y_t = y_true.copy() + 1
    y_p = y_pred.copy() + 1
    ...
    def objective(threshold):
        ...
        return -f1_score(sorted_y_t, classified_preds, average='weighted')

    # Bayesian optimization: search within [0.25, 1.75]
    res = gp_minimize(objective, [Real(0.25, 1.75)], n_calls=25, random_state=42)

    optimized_threshold = res.x[0]
...
def make_hist2d(group_num, steps, ins, outs, scaler, event_type, file_path, mask=None, lower=None, upper=None):
    ...
    for step in range(steps):
        ...
        if step == 3:
            bins = 3
            outputs[:, group_num*steps+step] = optimize_thresholds_bayes(inputs[:,group_num*steps+step], outputs[:,group_num*steps+step], mask=mask)
        else:
            bins = 30
        varname = names[group_num*steps+step]
        heatmap, xedges, yedges = np.histogram2d(inputs[:,group_num*steps+step],
...
class AdamWithSaturatingMomentumAndDecay(Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), max_beta1=0.99, 
                 epochs_to_saturate=100, batches_per_epoch=1, weight_decay=0, 
                 lr_decay=0.1, min_lr=1e-6, resume_epoch=0, eps=1e-8):
        if lr < 0.0:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if weight_decay < 0.0:
            raise ValueError("Invalid weight decay value: {}".format(weight_decay))
        
        defaults = dict(lr=lr, betas=betas, max_beta1=max_beta1, 
                        epochs_to_saturate=epochs_to_saturate, 
                        batches_per_epoch=batches_per_epoch, weight_decay=weight_decay, 
                        lr_decay=lr_decay, min_lr=min_lr, eps=eps, resume_epoch=resume_epoch)
        super(AdamWithSaturatingMomentumAndDecay, self).__init__(params, defaults)
        ...
        for group in self.param_groups:
            ...
            max_beta1 = group['max_beta1']
            beta1_step = (max_beta1 - group['betas'][0]) / steps_to_saturate

            # Adjust beta1 for resumed training
            group['betas'] = (min(group['betas'][0] + beta1_step * resumed_steps, max_beta1), group['betas'][1])
            # Adjust learning rate for resumed training
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            # Increment beta1 and decay learning rate after the step
            group['betas'] = (min(group['betas'][0] + beta1_step, max_beta1), group['betas'][1])
